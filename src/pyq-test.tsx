...
// ─── Types ────────────────────────────────────────────────────────────────────
// Same shapes as the Mock Test module, since both are driven off the same
// Google Sheet workbook — just different tabs (e.g. "PYQ 2023 Slot 1" vs
// "Mock Test 1"). Renamed at the type level (PYQPaper / PYQQuestion) so the
// two features don't get confused, but the fields line up 1:1.
...
interface PYQPaper {
  id: string;
  name: string; // e.g. "CAT 2023 Slot 1"
...
function isTitaQuestion(q: PYQQuestion) {
...
export default function PYQTest({ user }: { user: any }) {
  const [view, setView] = useState<
    "list" | "instructions" | "section-intro" | "test" | "section-break" | "result"
  >("list");
  const [availablePapers, setAvailablePapers] = useState<PYQPaper[]>([]);
  const [attempts, setAttempts] = useState<Record<string, PYQResult>>({});
  ...
  const paperStartRef = useRef<number>(0);
  ...
  const loadPapers = async () => {
    setLoading(true);
    try {
      const [papers, prevResults] = await Promise.all([
        apiRequest("/pyq-papers"),
        apiRequest("/pyq-results"),
      ]);
      // Papers render in whatever order the sheet returns them in — the sheet
      // is the source of truth for ordering (e.g. newest slot first, or
      // however the admin has arranged the tabs/rows).
      setAvailablePapers(papers || []);
      const map: Record<string, PYQResult> = {};
      (prevResults || []).forEach((r: PYQResult) => {
        map[r.paperId] = r;
      });
      setAttempts(map);
    } catch (err: any) {
  ...
  const sectionQuestions = useCallback(
    (paper: PYQPaper | null, section: SectionName) => {
      if (!paper?.questions) return [];
      return paper.questions.filter((q) => q.section === section);
    },
    []
  );
  ...
  useEffect(() => {
    if (!selectedPaper || view !== "test") return;
    const qs = sectionQuestions(selectedPaper, SECTION_ORDER[sectionIdx]);
    const q = qs[currentIdx];
    if (q?.passageId && selectedPaper.passages) {
      setActivePassage(selectedPaper.passages.find((p) => p.id === q.passageId) || null);
    } else {
      setActivePassage(null);
    }
  }, [currentIdx, sectionIdx, selectedPaper, view, sectionQuestions]);
  ...
  }, [currentIdx, sectionIdx, selectedPaper, view]); // eslint-disable-line react-hooks/exhaustive-deps

  const startPaper = async (paper: PYQPaper) => {
    if (attempts[paper.id]) {
      setPaperLoading(true);
      try {
        const fullPaper = await apiRequest(`/pyq-paper/${paper.id}`);
        setSelectedPaper(fullPaper);
        setResult(attempts[paper.id]);
        setAnswers(attempts[paper.id].studentAnswers || {});
        setView("result");
      } catch {
        toast.error("Failed to load paper for review");
      } finally {
    ...
    try {
      ...
      if (!fullPaper?.questions?.length) {
        toast.error("This paper has no questions yet. Please check the sheet data.");
        return;
      }
      setSelectedPaper(fullPaper);
      setView("instructions");
    } catch (err: any) {
      toast.error("Failed to load paper questions. Check server connection.");
    } finally {
      setPaperLoading(false);
    }
  };
  ...
  const beginSection = (idx: number) => {
    ...
    const qs = sectionQuestions(selectedPaper, SECTION_ORDER[idx]);
    if (!qs.length) {
    ...
    setTitaDraft("");
    setFlagged(new Set());
    setTimeLeft((selectedPaper.sectionDurationMinutes || 40) * 60);
    setSubmitted(false);
    setView("test");
  };

  const startWholePaper = () => {
    if (!selectedPaper) return;
    setAnswers({});
    paperStartRef.current = Date.now();
    setSectionIdx(0);
    beginSection(0);
  };
  ...
  const goToIdx = (newIdx: number, questions: PYQQuestion[]) => {
  ...
  }, [submitted, sectionIdx, selectedPaper, currentIdx, titaDraft]);
  ...
  const finalizePaper = useCallback(async () => {
    if (!selectedPaper) return;

    const sectionResults: SectionResult[] = SECTION_ORDER.map((sec) => {
      const qs = sectionQuestions(selectedPaper, sec);
      ...
      return {
        ...
        score,
        scaledScore,
        timeSpent: (selectedPaper.sectionDurationMinutes || 40) * 60,
      };
    });
    ...
    const timeSpent = Math.round((Date.now() - paperStartRef.current) / 1000);

    const payload: PYQResult = {
    ...
    try {
      await apiRequest("/pyq-results", {
        method: "POST",
        body: JSON.stringify(payload),
      });
      setResult(payload);
      setAttempts((prev) => ({ ...prev, [selectedPaper.id]: payload }));
      toast.success("PYQ paper submitted!");
    } catch {
  ...
  }, [selectedPaper, answers, sectionQuestions]);
      ...
      <div className="space-y-8">
        <header>
          ...
          <p className="text-muted-foreground mt-1">
            Actual CAT papers · VARC + DILR + QA · attempted exactly as they appeared on exam day
          </p>
        </header>
        ...
        {availablePapers.length === 0 ? (
          <div className="py-20 flex flex-col items-center justify-center text-center border border-dashed rounded-2xl bg-background">
            ...
            <h3 className="font-bold text-lg">No PYQ Papers Available</h3>
            <p className="text-muted-foreground max-w-sm mt-1">
              Your admin hasn't published any previous year papers yet. Check back soon.
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {availablePapers.map((p) => {
              const attempted = attempts[p.id];
              return (
                <Card key={p.id} className="hover:shadow-md transition-all border-t-4 border-t-amber-500">
                  <CardHeader className="pb-2">
                    ...
                    {(p.year || p.slot || p.examDate) && (
                      <div className="flex items-center gap-1 text-xs text-muted-foreground">
                        <CalendarDays size={12} />
                        <span>{[p.year, p.slot, p.examDate].filter(Boolean).join(" · ")}</span>
                      </div>
                    )}
                  </CardHeader>
                  <CardContent className="space-y-3">
                    <div className="flex gap-4 text-xs text-muted-foreground">
                      <span className="flex items-center gap-1">
                        <Clock size={12} /> {p.totalDurationMinutes || 120} min
                      </span>
                      <span className="flex items-center gap-1">
                        <BookOpen size={12} /> {p.questions?.length ?? "–"} Qs
                      </span>
                    </div>
                    {attempted && (
                      <div className="p-3 rounded-xl bg-amber-50 border border-amber-200">
                        <div className="flex gap-4 text-center">
                          <div className="flex-1">
                            ...
                            <p className="text-xl font-black text-amber-700">{attempted.overallScaledScore}</p>
                          </div>
                    ...
                    >
                      {paperLoading && selectedPaper?.id === p.id
                        ? "Loading..."
                        : attempted
                        ? "Review Attempt"
                        : "Attempt Paper"}
  ...
  // ── INSTRUCTIONS ─────────────────────────────────────────────────────────────
  if (view === "instructions" && selectedPaper) {
    return (
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        <Card className="border-2 border-amber-200">
          <CardHeader className="bg-amber-50 rounded-t-xl">
            ...
            <CardTitle className="text-2xl">{selectedPaper.name}</CardTitle>
            {(selectedPaper.year || selectedPaper.slot || selectedPaper.examDate) && (
              <div className="flex items-center gap-1 text-xs text-muted-foreground">
                <CalendarDays size={12} />
                <span>{[selectedPaper.year, selectedPaper.slot, selectedPaper.examDate].filter(Boolean).join(" · ")}</span>
              </div>
            )}
            <div className="flex items-center justify-between text-xs mt-3 pt-3 border-t border-border">
              ...
              <span className="font-semibold text-emerald-600">
                {(selectedPaper.studentsAttempted || 0).toLocaleString()}
              </span>
            </div>
          </CardHeader>
          <CardContent className="pt-6 space-y-6">
            <div className="grid grid-cols-3 gap-4 text-center">
              {[
                ["Total Duration", `${selectedPaper.totalDurationMinutes || 120} min`],
                ["Sections", "3"],
                ["Marking", "+3 / –1"],
              ].map(([label, val]) => (
            ...
            <div className="grid grid-cols-3 gap-3">
              {SECTION_ORDER.map((sec) => {
                const meta = SECTION_META[sec];
                const count = sectionQuestions(selectedPaper, sec).length;
                return (
                  <div key={sec} className={`p-3 rounded-xl border ${meta.lightColor} ${meta.borderColor} text-center`}>
                    ...
                    <p className="text-[10px] text-muted-foreground">questions · {selectedPaper.sectionDurationMinutes || 40} min</p>
                  </div>
                );
              })}
            </div>
            ...
            <div className="space-y-3">
              ...
              {[
                "This is an actual past CAT paper, reproduced with 3 sections, each individually timed. You cannot go back to a previous section once it ends.",
                ...
                "After the final section, you'll see your composite score, percentile estimate, and a full review with the original explanations.",
              ].map((rule, i) => (
                <div key={i} className="flex gap-3 text-sm">
                  <span className="w-5 h-5 shrink-0 rounded-full bg-amber-500 text-white flex items-center justify-center text-[10px] font-bold mt-0.5">
                    {i + 1}
                  </span>
            ...
            <Button size="lg" className="w-full" onClick={startWholePaper}>
              Begin Paper · {selectedPaper.totalDurationMinutes || 120} min
            </Button>
          </CardContent>
        </Card>
      </div>
  ...
  // ── SECTION BREAK ─────────────────────────────────────────────────────────────
  if (view === "section-break" && selectedPaper) {
          ...
          <CardContent className="pt-8 pb-8 text-center space-y-6">
            ...
            <div className={`p-5 rounded-2xl ${nextMeta.lightColor} border ${nextMeta.borderColor}`}>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                {sectionQuestions(selectedPaper, next).length} questions ·{" "}
                {selectedPaper.sectionDurationMinutes || 40} minutes
              </p>
            </div>
  ...
  // ── TEST VIEW ─────────────────────────────────────────────────────────────────
  if (view === "test" && selectedPaper) {
    const currentSection = SECTION_ORDER[sectionIdx];
    const questions = sectionQuestions(selectedPaper, currentSection);
    if (!questions.length) {
      return (
        <div className="flex flex-col items-center justify-center py-24 gap-4">
          <p className="text-muted-foreground">No questions found for this section.</p>
          <Button onClick={() => setView("list")}>Back to Papers</Button>
        </div>
      );
    }
    const currentQ = questions[currentIdx];
          ...
          <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-4">
              ...
              <Button size="sm" variant="destructive" onClick={handleSectionEnd}>
                {sectionIdx === SECTION_ORDER.length - 1 ? "Submit Paper" : "Submit Section"}
              </Button>
            </div>
          </div>
  ...
  // ── RESULT VIEW ───────────────────────────────────────────────────────────────
  if (view === "result" && result && selectedPaper) {
    if (reviewMode) {
      const section = reviewSection || SECTION_ORDER[0];
      const meta = SECTION_META[section];
      const questions = sectionQuestions(selectedPaper, section);
        ...
        <div className="max-w-4xl mx-auto space-y-6">
          <div className="flex items-center justify-between gap-3 flex-wrap">
            <div className="flex items-center gap-3">
              ...
              <span className="font-bold">{selectedPaper.name} · Review</span>
            </div>
          ...
          <div className="space-y-4">
            {questions.map((q, idx) => {
              ...
              const passage = q.passageId && selectedPaper.passages
                ? selectedPaper.passages.find((p) => p.id === q.passageId)
                : null;
      ...
      <div className="max-w-3xl mx-auto space-y-8">
        ...
        <Card className="border-2 border-amber-200 overflow-hidden">
          <div className="bg-gradient-to-r from-amber-500 to-orange-500 px-6 py-5 text-white">
            <p className="text-sm font-bold uppercase tracking-widest opacity-80">Previous Year Paper Result</p>
            <h2 className="text-2xl font-black mt-1">{selectedPaper.name}</h2>
            {(selectedPaper.year || selectedPaper.slot || selectedPaper.examDate) && (
              <p className="text-xs opacity-80 mt-1">
                {[selectedPaper.year, selectedPaper.slot, selectedPaper.examDate].filter(Boolean).join(" · ")}
              </p>
            )}
            <div className="flex items-center justify-between text-xs mt-3 pt-3 border-t border-white/20">
              ...
              <span className="font-semibold">{(selectedPaper.studentsAttempted || 0).toLocaleString()}</span>
            </div>
          </div>
          <CardContent className="pt-6">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
              {[
                { label: "Composite Score", val: result.overallScaledScore, color: "text-amber-700", big: true },
        ...
        <Button className="w-full gap-2" onClick={() => setView("list")}>
          <BarChart3 size={16} /> Back to Papers
        </Button>
      </div>
    );
  }
